...
class valr extends valr$1["default"] {
    ...
    async watchOrderBookForSymbols(symbols, limit = undefined, params = {}) {
        ...
        if (symbols === undefined || marketIds === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchOrderBookForSymbols() requires valid symbol list');
        }
    ...
    async watchTickers(symbols = undefined, params = {}) {
        ...
        const marketIds = this.marketIds(symbols);
        if (symbols === undefined || marketIds === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchTickers() requires valid symbol list');
        }
        const url = this.urls['api']['ws']['trade'];
        const client = this.authenticate(url);
        const messageHashes = [];
    ...
    async watchTradesForSymbols(symbols, since = undefined, limit = undefined, params = {}) {
        ...
        const marketIds = this.marketIds(symbols);
        if (symbols === undefined || marketIds === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchTradesForSymbols() requires valid symbol list');
        }
        const url = this.urls['api']['ws']['trade'];
        const client = this.authenticate(url);
        const messageHashes = [];
    ...
    async watchOHLCVForSymbols(symbolsAndTimeframes, since = undefined, limit = undefined, params = {}) {
        ...
        for (let i = 0; i < symbolsAndTimeframes.length; i++) {
            ...
            if ((timeframe === undefined) || !(timeframe in this.timeframes)) {
                throw new errors.ArgumentsRequired(this.id + ' watchOHLCVForSymbols() requires supported timeframe option');
            }
    ...
    parseWsBalance(balanceWs) {
        ...
        const lendBorrow = Precise["default"].stringAdd(this.safeString(balanceWs, 'lendReserved'), this.safeString(balanceWs, 'borrowReserved'));
        const used = Precise["default"].stringAdd(lendBorrow, this.safeString(balanceWs, 'reserved'));
        const debt = Precise["default"].stringSub(this.safeString(balanceWs, 'borrowedAmount'), this.safeString(balanceWs, 'borrowReserved'));
        if (code !== undefined) {
            result[code] = {
    ...
    async watchMyTrades(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        this.authenticate(url);
        const trades = await this.watch(url, messageHash);
        if (this.newUpdates) {
    ...
    parseWsOrder(order, market = undefined) {
        ...
        if (remaining === undefined && filledPercentage !== undefined) {
            filled = Precise["default"].stringDiv(Precise["default"].stringMul(amount, filledPercentage), '100');
        }
