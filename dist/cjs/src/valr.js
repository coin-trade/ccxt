...
var valr$1 = require('./abstract/valr.js');
 ...
 * @augments Exchange
 */
class valr extends valr$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'api': {
                ...
                'private': {
                    'get': [
                        ...
                        'wallet/crypto/withdraw/history',
            ...
            'exceptions': {
                'exact': {
                    '-1': errors.OrderNotFound,
                    '-12007': errors.InvalidOrder,
                    '-11502': errors.DuplicateOrderId,
                    '-113': errors.BadRequest, // "code":-113,"message":"Invalid JSON payload"
                },
                'broad': {
                    'Internal transfer did not succeed From account does not have sufficient balance for transfer': errors.InsufficientFunds,
                    'Internal transfer did not succeed A subaccount can only transfer funds from itself': errors.BadRequest,
                    'Invalid Request, please check your request and try again': errors.BadRequest,
                },
            },
        });
    }
    checkRequiredSymbolArgument(methodName, symbol) {
        if (symbol === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' ' + methodName + '() requires valid symbol name');
        }
        if (typeof symbol !== 'string') {
            throw new errors.ArgumentsRequired(this.id + ' ' + methodName + '() requires valid string name for symbol');
        }
        const market = this.safeMarket(symbol);
        if (market === undefined) {
            throw new errors.BadSymbol(this.id + ' ' + methodName + '() found no valid market for symbol: ' + symbol);
        }
    }
    checkRequiredCurrencyCodeArgument(methodName, code) {
        if (code === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' ' + methodName + '() requires valid currency code name');
        }
        const currency = this.safeCurrency(code);
        if (currency['id'] === undefined) {
            throw new errors.BadRequest(this.id + ' ' + methodName + '() found no valid currency ID for currency code: ' + code);
        }
    }
    isFiat(code) {
    ...
    parseBalance(balances) {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            const lendBorrow = Precise["default"].stringAdd(this.safeString(balance, 'lendReserved'), this.safeString(balance, 'borrowReserved'));
            const used = Precise["default"].stringAdd(lendBorrow, this.safeString(balance, 'reserved'));
            const debt = Precise["default"].stringSub(this.safeString(balance, 'borrowedAmount'), this.safeString(balance, 'borrowReserved'));
            result[code] = {
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (side !== 'buy' && side !== 'sell') {
            throw new errors.InvalidOrder(this.id + ' createOrder() - "side" must be either "buy" or "sell".');
        }
        const body = {
        ...
        if (type === 'market') {
            ...
            throw new errors.InvalidOrder(this.id + ' createOrder() - "type" must be either "market" or "limit" to create an order.');
        }
        const timestamp = this.parseDate(this.safeString(this.last_response_headers, 'Date'));
    ...
    async fetchTradingFees(params = {}) {
        ...
        if (!Array.isArray(response)) {
            throw new errors.NullResponse(this.id + ' ' + 'fetchTradingFees() received incorrect response');
        }
        const result = {};
        for (let i = 0; i < response.length; i++) {
            ...
            if (('makerPercentage' in tradeFee) && ('takerPercentage' in tradeFee)) {
                ...
                makerStr = (makerStr !== undefined) ? Precise["default"].stringDiv(makerStr, '100') : undefined;
                takerStr = (takerStr !== undefined) ? Precise["default"].stringDiv(takerStr, '100') : undefined;
                result[symbol] = {
    ...
    parseBorrowInterest(info, market = undefined) {
        ...
        const interestRate = Precise["default"].stringDiv(interestAmount, amount, 9);
        return {
            'account': undefined,
            'currency': code,
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            const signature = this.hmac(this.base64ToBinary(payloadBase64), this.base64ToBinary(this.stringToBase64(this.secret)), sha512.sha512, 'hex');
            signHeaders = {
                // 'Content-Type': 'application/json',
